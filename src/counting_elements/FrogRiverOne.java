...
public class FrogRiverOne {
    public static void main(String[] args) {
        solution (3, new int[]{1, 3, 1, 3, 2, 1, 3});
        solution (1, new int[]{1});


        // let's try a really big array
        Random random = new Random();
        int[] bigArray = new int[10_000_000];

        for (int i = 0; i < bigArray.length; i++){
            bigArray[i] = random.nextInt(100_000);
        }

        long start = System.currentTimeMillis();

        solution(100_000, bigArray);

        long finish = System.currentTimeMillis();

        System.out.println("Real time: " + (finish - start) + " ms");
